...
_defineProperties GridFw.prototype,
	logLevel:
		...
		set: (level)->
			...
			contextProto.debug=		if levelIndex is 0 then settings.log_debug else LOG_IGNORE
			contextProto.info=		if levelIndex <= 1 then settings.log_info else LOG_IGNORE
			contextProto.warn=		if levelIndex <= 2 then settings.log_warn else LOG_IGNORE
			contextProto.error=		if levelIndex <= 3 then settings.log_error else LOG_IGNORE
			contextProto.fatalError=if levelIndex <= 4 then settings.log_fatalError else LOG_IGNORE
			return
