...
```javascript
    ...
    app.get('/an/*other/sta**tic/?*path', function(ctx){});// the last "star" must be escaped with "?", so it will not be considered as a wildcard param
    app.get('/st::atic/?:path/??to/src', function(ctx){}); // when a word after "/" starts with ":" or "?", it must be escaped with "?", so it will not be considered as a path param
